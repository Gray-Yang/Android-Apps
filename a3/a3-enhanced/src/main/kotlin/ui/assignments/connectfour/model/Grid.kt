...
class Grid(width: Int, height: Int, private val length: Int) {
    ...
    /**
     * Returns the larger of two Pair<Player, Int>. why do we need to use this func
     */
     ...
     * @param list the list
     * @return a [Pair<Player, Int>] that holds the length of the longest chain, and which player's pieces form this chain. 有用如果回来是4 且 有 player就出结果
     */
     ...
     * @return player who has won the game, or [Player.NONE] if no-one has won the game yet. every step call this to see whether there is a winer
     */
    fun hasWon() : Player {
     ...
     * @return draw-condition has occurred if no winner check if the game is draw
     */
    fun hasDraw() : Boolean {
     ...
     * 如果人松手那就drop那个东西下去看看是不是成功 如果成功returns piece
     */
